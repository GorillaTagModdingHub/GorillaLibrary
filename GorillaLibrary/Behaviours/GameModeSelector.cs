...
namespace GorillaLibrary.Behaviours
{
    ...
    internal class GameModeSelector : MonoBehaviour
    {
        ...
        public readonly Dictionary<bool, List<Models.GameModeWrapper>> SelectorGameModes = [];
        ...
        private List<ModeSelectButton> currentButtons;

        public async void Awake()
        {
            Layout = GetComponent<GameModeSelectorButtonLayout>();

            Zone = (GTZone)AccessTools.Field(typeof(GameModeSelectorButtonLayout), "zone").GetValue(Layout);
            currentButtons = (List<ModeSelectButton>)AccessTools.Field(typeof(GameModeSelectorButtonLayout), "currentButtons").GetValue(Layout);

            while (currentButtons.Count == 0)
                await Task.Yield();

            ModifySelectionButtons();
            CreatePageButtons(currentButtons.First().gameObject);
        ...
        public List<Models.GameModeWrapper> GetSelectorGameModes()
        {
            bool sessionIsPrivate = NetworkSystem.Instance.SessionIsPrivate;

            if (SelectorGameModes.TryGetValue(sessionIsPrivate, out List<Models.GameModeWrapper> gameModeList))
                return gameModeList;
            ...
            {
                if (GameModeManager.Instance.DefaultGameModesPerMode.TryGetValue(modesForZone[i], out Models.GameModeWrapper gamemode))
                {
            ...
            // Custom gamemodes
            if (GameModeManager.Instance.CustomGameModes is List<Models.GameModeWrapper> customGameModes)
            {
            ...
            GorillaPressableButton superToggleButton = (GorillaPressableButton)AccessTools.Field(typeof(GameModeSelectorButtonLayout), "superToggleButton").GetValue(Layout);
        ...
        public void CheckGameMode()
        {
            ...
            {
                SetDefaultMode(((GameModeType)AccessTools.Field(typeof(CustomMapModeSelector), "defaultGamemodeForLoadedMap").GetValue(null)).GetName());
                return;
            }
        ...
        public void SetDefaultMode(string defaultMode)
        {
            ...
            if (isModded)
            {
                ...
                if (isBaseMode && GameModeManager.Instance.ModdedGamemodesPerMode.TryGetValue(modeType, out Models.GameModeWrapper moddedGamemode)) defaultMode = moddedGamemode.ID;
            }
        ...
        {
            List<Models.GameModeWrapper> allGamemodes = GetSelectorGameModes();
            List<Models.GameModeWrapper> shownGamemodes = [.. allGamemodes.Skip(CurrentPage * PageCapacity).Take(PageCapacity)];

            for (int i = 0; i < currentButtons.Count; i++)
            {
                Models.GameModeWrapper gamemode = shownGamemodes.ElementAtOrDefault(i);
                bool hasMode = gamemode != null && gamemode.ID != null && gamemode.ID.Length != 0;

                ModeSelectButton button = currentButtons[i];
        ...
        {
            PageCapacity = currentButtons.Count(button => button.gameObject.activeSelf);

            foreach (var mb in currentButtons)
            {
                if (moddedSelectionButtons.Contains(mb)) continue;
                moddedSelectionButtons.Add(mb);

                TMP_Text gamemodeTitle = (TMP_Text)AccessTools.Field(typeof(ModeSelectButton), "gameModeTitle").GetValue(mb);
